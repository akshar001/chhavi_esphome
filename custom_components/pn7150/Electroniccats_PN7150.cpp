...
{
    Serial.println("In ProcessCard Mode");
    ...
    (void)writeData(Answer, sizeof(Answer));
    getMessage(2000);
...
{
    Serial.println("in PresenceCheck");
    bool status;
    uint8_t i;
    ...
    Serial.println("out of presence check");
}
...
void Electroniccats_PN7150::FlushReception(void)
{
...
void NdefPull_Cb(unsigned char *pNdefMessage, unsigned short ReceivedSize, unsigned int NdefMessageSize)
{
    ...
    if (gPendingReception)
    {
        ...
        {
            //Serial.print("--- Receiving Ndef fragmented message, pending reception of %d bytes\n", gPendingReception);
            return;
        }
        else
        {
            Serial.print("--- Received complete Ndef fragmented message\n");
    ...
    else if(NdefMessageSize > ReceivedSize)
    {
        ...
        {
            Serial.print("--- Provisioned buffer size too small \n");
            return;
        }
        memcpy(gNdefBuffer, pNdefMessage, ReceivedSize);
        gPendingReception = NdefMessageSize - ReceivedSize;
        //Serial.print("--- Receiving Ndef fragmented message (size = %d bytes), pending reception of %d bytes\n", NdefMessageSize, gPendingReception);
        return;
    }
